...
import { UI_EVENTS, UI_COLORS } from "../ui_const"; // <--- UI_COLORS импортирован
...
FirstHeroSelectLayout.prototype.createConfirmPanel = function(){
    ...
    const text = new TextBlock('select_your_hero_text', this.polylang.t(TEXT_KEYS.SELECT_FIRST_HERO))
    this.registerText( TEXT_KEYS.SELECT_FIRST_HERO, text);
    ...
    text.left = 0;
    text.top = 0;
    text.color = UI_COLORS.BUTTON_TEXT;
    // text.background = '#eaee00ffff'
    container.addControl(text);

    const confirm_btn = Button.CreateSimpleButton('confirm_first_hero', this.polylang.t(TEXT_KEYS.CONFIRM));
     this.registerText( TEXT_KEYS.CONFIRM, confirm_btn.textBlock);
    ...
    confirm_btn.textBlock.fontSize = 36;
    confirm_btn.background = UI_COLORS.BUTTON_DEFAULT;
    confirm_btn.color = UI_COLORS.BUTTON_BORDER;
    confirm_btn.textBlock.color = UI_COLORS.BUTTON_TEXT;
    confirm_btn.onPointerEnterObservable.add((item)=>{item.background = UI_COLORS.BUTTON_HOVER});
    confirm_btn.onPointerOutObservable.add((item)=>{item.background = UI_COLORS.BUTTON_DEFAULT});

    confirm_btn.onPointerClickObservable.add(()=>{
        if(this.gameDataManager.firstSelectedHero){
            this.uiCommandsListener?.(
                UI_EVENTS.SELECT_FIRST_HERO_CONFIRM,
                {unit_id: this.gameDataManager.firstSelectedHero}
            )
        }else{
            alert(this.polylang.t(TEXT_KEYS.SELECT_HERO_WARN));
        }
    });
...
FirstHeroSelectLayout.prototype._createHeroButton = function(heroId) {
    ...
    container_btn.background = UI_COLORS.BUTTON_DEFAULT;
    container_btn.color = UI_COLORS.BUTTON_BORDER;
    container_btn.textBlock.color = UI_COLORS.BUTTON_TEXT;
    container_btn.onPointerEnterObservable.add((item)=>{item.background = UI_COLORS.BUTTON_HOVER});
    container_btn.onPointerOutObservable.add((item)=>{item.background = UI_COLORS.BUTTON_DEFAULT});
    ...
    if (textBlock instanceof TextBlock) {
        ...
        textBlock.color = UI_COLORS.BUTTON_TEXT;
    }
